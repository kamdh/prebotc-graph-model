...
if docombined
    ...
    for gEidx = 1:4
        for gIidx = 1:4
            ...
            pltGStr = sprintf('gE_%1.0f_gI_%1.0f', gE, gI);

            figure
            myPcolor(X,Y, chiArray(:,:,gEidx, gIidx));
            titlestr=sprintf('synchrony \\chi; gE = %1.0f, gI = %1.0f', ...
                             gE,gI);
            ...
            colorbar
            colormap('gray')
            plt = [plotDir, '/', plotGStr, '_chi.eps']
            print('-deps', plt)

            figure
            myPcolor(X,Y, dutyCycle(:,:,gEidx, gIidx))
            titlestr = sprintf('duty cycle; gE = %1.0f, gI = %1.0f', gE,gI);
            title(titlestr, 'fontsize', 32)
            xlabel('\langle k \rangle', 'fontsize', 24)
            ylabel('p_I','fontsize', 24)
            colorbar
            colormap('gray')
            plt = [plotDir, '/', plotGStr, '_duty_cycle.eps']
            print('-deps', plt)

            figure
            myPcolor(X,Y, fMax(:,:,gEidx, gIidx))
            titlestr=sprintf('peak frequency (Hz); gE = %1.0f, gI = %1.0f',...
                             gE,gI);
            title(titlestr, 'fontsize', 32)
            ...
            colorbar
            colormap('gray')
            plt = [plotDir, '/', plotGStr, '_peak_freq.eps']
            print('-deps', plt)

            figure
            myPcolor(X,Y, lag(:,:,gEidx, gIidx))
            titlestr=sprintf('dominant period (s); gE = %1.0f, gI = %1.0f',...
                             gE,gI);
            title(titlestr, 'fontsize', 32)
            xlabel('\langle k \rangle', 'fontsize', 24)
            ylabel('p_I','fontsize', 24)
            colorbar
            colormap('gray')
            plt = [plotDir, '/', plotGStr, '_lag.eps']
            print('-deps', plt)

            figure
            myPcolor(X,Y, muB(:,:,gEidx, gIidx) / 1000)
            titlestr=sprintf(['mean burst duration (s); gE = %1.0f, ' ...
                              'gI = %1.0f'], ...
                             gE,gI);
            title(titlestr, 'fontsize', 32)
            ...
            colorbar
            colormap('gray')
            plt = [plotDir, '/', plotGStr, '_mean_burst.eps']
            print('-deps', plt)

            figure
            myPcolor(X,Y, muIBI(:,:,gEidx, gIidx) / 1000)
            titlestr=sprintf('mean IBI (s); gE = %1.0f, gI = %1.0f',...
                             gE,gI);
            title(titlestr, 'fontsize', 32)
            xlabel('\langle k \rangle','fontsize', 24)
            ylabel('p_I','fontsize', 24)
            colorbar
            colormap('gray')
            plt = [plotDir, '/', plotGStr, '_mean_IBI.eps']
            print('-deps', plt)

            figure
            myPcolor(X,Y, cvIBI(:,:,gEidx, gIidx))
            titlestr=sprintf('CV of IBIs; gE = %1.0f, gI = %1.0f',...
                             gE,gI);
            title(titlestr, 'fontsize', 32)
            xlabel('\langle k \rangle','fontsize', 24)
            ylabel('p_I','fontsize', 24)
            colorbar
            colormap('gray')
            plt = [plotDir, '/', plotGStr, '_cv_IBIs.eps']
            print('-deps', plt)

            figure
            myPcolor(X,Y, cvB(:,:,gEidx, gIidx))
            titlestr=sprintf('CV of burst durations; gE = %1.0f, gI = %1.0f',...
                             gE,gI);
            title(titlestr, 'fontsize', 32)
            xlabel('\langle k \rangle','fontsize', 24)
            ylabel('p_I','fontsize', 24)
            colorbar
            colormap('gray')
            plt = [plotDir, '/', plotGStr, '_cv_bursts.eps']
            print('-deps', plt)

            figure
            myPcolor(X,Y, op_angle_mean(:,:,gEidx, gIidx))
            titlestr=sprintf('mean OP phase; gE = %1.0f, gI = %1.0f',...
                             gE,gI);
            title(titlestr, 'fontsize', 32)
            xlabel('\langle k \rangle','fontsize', 24)
            ylabel('p_I','fontsize', 24)
            colorbar
            colormap('gray')
            plt = [plotDir, '/', plotGStr, '_op_angle_mean.eps']
            print('-deps', plt)
            ...
            colorbar
            colormap('gray')
            plt = [plotDir, '/', plotGStr, '_op_angle_std.eps']
            print('-deps', plt)
